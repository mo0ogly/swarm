...
func (s *Store) mutateWithHook(id, kind, event string, expected int, request []byte, fn func(*Work) error, hook func(*sql.Tx, *Work) error) (Work, error) {
	...
	defer tx.Rollback()
	if kind == "role.model" || kind == "task.model" || kind == "quotas.configure" || kind == "budget.configure" || kind == "managed.integrated" || kind == "task.attempt-extension" || kind == "task.corrective-recovery" {

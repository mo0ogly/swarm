...
func (s *Store) renderDashboard(work string, c *consoleState, width, height int, input string) string {
	...
	if width < 64 || height < 20 {
		return strings.Join([]string{clip(uiText("SWARM — terminal trop petit (minimum 64 × 20)"), width), clip(uiText("Agrandir ou relancer avec --plain. q puis Entrée quitte."), width), clip("swarm> "+input, width)}, "\r\n")
	}
	lines := []string{fmt.Sprintf("SWARM  │ %s", w.Title), "", uiText("Sélectionnez une tâche puis Entrée pour agir.")}
	...
	for i := start; i < len(tasks) && len(tr) < topHeight-1; i++ {
		...
		tr = append(tr, fmt.Sprintf("%s %-10s %s · %s", marker, uiText(uiStatus(t.Status)), t.ID, t.Title))
	}
	...
	for i := start; i < len(agents) && len(ar) < topHeight-1; i++ {
		...
		ar = append(ar, fmt.Sprintf("%s %s %s · %s", marker, a.Provider, uiText(uiStatus(observedAgent(a))), a.TaskID))
	}
	if len(tr) == 0 {
		tr = append(tr, uiText("Aucune tâche. help pour créer une tâche."))
	}
	if len(ar) == 0 {
		ar = append(ar, uiText("Aucun agent correspondant."))
	}
	if width < 110 {
		if c.focus == 1 {
			lines = append(lines, panel(fmt.Sprintf("AGENTS · %d", len(agents)), ar, width-1, topHeight, true)...)
		} else {
			lines = append(lines, panel(fmt.Sprintf(uiText("TÂCHES · %d"), len(tasks)), tr, width-1, topHeight, c.focus == 0)...)
		}
	} else {
		left := (width - 3) / 2
		right := width - left - 3
		tp := panel(fmt.Sprintf(uiText("TÂCHES · %d"), len(tasks)), tr, left, topHeight, c.focus == 0)
		ap := panel(fmt.Sprintf("AGENTS · %d", len(agents)), ar, right, topHeight, c.focus == 1)
		for i := range tp {
			lines = append(lines, pad(tp[i], left)+" │ "+ap[i])
		}
	}
	detail := uiText("Sélectionner une tâche")
	criteria := ""
	if len(tasks) > 0 {
		t := tasks[c.taskCursor]
		gate := uiText("en attente")
		if t.Gate != nil {
			gate = gateLabel(&t) + uiText(" — bloquée/périmée")
			if s.validGate(&t) {
				gate = gateLabel(&t) + uiText(" — delivery valide")
		...
		if t.Status == "waived" {
			gate = uiText("DÉROGATION MANUELLE · gate : ") + gate
		}
		detail = uiText("Responsable : ") + t.Owner + " · Validation : " + gate
		criteria = uiText("LIVRABLE : ") + t.Deliverable + uiText(" | CRITÈRES : ") + strings.Join(t.Criteria, " ; ")
	}
	...
	if c.selected != "" {
		...
		if err == nil {
			logRows = append(logRows, uiText("Agent : ")+a.Provider+" · "+uiText(uiStatus(observedAgent(a))))
			if activeAgent(a) {
				logRows = append(logRows, uiText("Suivi reçu il y a ")+activityAge(a.Heartbeat)+uiText(" · durée ")+activityAge(a.Started))
				if a.Progress.PendingTools > 0 {
					action := a.Progress.Action
					if action == "" {
						action = a.Progress.LastTool + uiText(" — explication non fournie par cette tentative")
					}
					actionRows := readableWrap(uiText("EN COURS : ")+action, width-2)
					if len(actionRows) > 2 {
					...
					if a.Progress.Detail != "" {
						logRows = append(logRows, clip(uiText("Cible : ")+a.Progress.Detail, width-2))
					}
				} else {
					logRows = append(logRows, uiText("EN ATTENTE : prochaine sortie du fournisseur"))
					if a.Progress.Action != "" {
						logRows = append(logRows, clip(uiText("Dernière action : ")+a.Progress.Action, width-2))
					}
				}
			}
			if activeAgent(a) {
				if a.Progress.ToolCalls > 0 {
					logRows = append(logRows, fmt.Sprintf(uiText("Outils : %d appels / %d résultats · dernier : %s"), a.Progress.ToolCalls, a.Progress.ToolResults, a.Progress.LastTool))
				} else {
					logRows = append(logRows, uiText("Activité : ")+a.Activity)
				}
				if a.Progress.LastResult != "" {
					logRows = append(logRows, uiText("Dernier résultat reçu il y a ")+activityAge(a.Progress.LastResult))
				}
			} else {
				logRows = append(logRows, readableWrap(uiText("Fin : ")+a.Activity, width-1)...)
				if t, e := w.task(a.TaskID); e == nil {
					next := readableWrap(uiText("À faire : ")+t.Next, width-1)
					if len(next) > 2 {
						next = next[:2]
					}
			...
			if a.Progress.Degraded != "" {
				logRows = append(logRows, uiText("Surveillance incomplète : chronométrage par outil suspendu."))
			}
	...
	if len(logRows) == 0 {
		logRows = append(logRows, uiText("Tab vers AGENTS, puis flèches pour choisir un agent et lire son journal."))
	}
	if c.frozen {
		lines[2] = uiText("AFFICHAGE FIGÉ — commande freeze pour reprendre le suivi.")
	}
	...
	lines = append(lines, panel(uiText("SUIVI EN DIRECT / ACTIVITÉ"), logRows, width, logHeight, c.focus == 2)...)
	lines = append(lines, fmt.Sprintf("%d agents · %s · %s", len(agents), map[bool]string{true: uiText("Départs suspendus"), false: uiText("Départs autorisés")}[s.paused(work)], map[bool]string{true: uiText("Capture activée"), false: uiText("Capture désactivée")}[c.capture]), uiText("↑↓ tâche · Entrée actions · d détail · ? aide · t thème · q"), strings.ReplaceAll(c.message, "\n", " | "), "swarm> "+input)
	for i := range lines {
		lines[i] = clip(lines[i], width-1)
	}
	if consoleBinaryReplaced() {
		lines[len(lines)-2] = clip(uiText("Mise à jour installée : q puis relancer swarm console."), width-1)
	}

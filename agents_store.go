...
func (s *Store) agent(id string) (Agent, error) {
	var a Agent
	var b []byte
	e := s.db.QueryRow("SELECT body FROM agents WHERE id=?", id).Scan(&b)
	if e == nil {
...
func (s *Store) agents(work string) ([]Agent, error) {
	rows, e := s.db.Query("SELECT body FROM agents WHERE work_id=? ORDER BY rowid DESC LIMIT 200", work)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *Store) prepare(work string, r Launch) (Agent, bool, error) {
	...
	if overlaps > 0 {
		return a, false, fmt.Errorf("workspace occupé ou recouvrant un workspace actif")
	}
	originalNext := t.Next
	...
	a = Agent{Limits: limits, ID: r.EventID, WorkID: work, TaskID: r.TaskID, Attempt: t.Attempts[len(t.Attempts)-1].ID, Provider: r.Provider, Role: r.Role, Parent: r.Parent, Previous: r.Previous, CWD: cwd, Status: "queued", Activity: "Lancement demandé ; processus non confirmé", Started: now(), Host: hostIdentity(), Timeout: r.Timeout, Capture: r.Capture, Prompt: prompt, Command: p.Command, Args: p.Args, Env: p.Env}
	b, _ := json.Marshal(a)
...
func (s *Store) stopAgent(id string) error {
	...
	defer tx.Rollback()
	if _, e = tx.Exec("UPDATE agents SET desired='stop' WHERE id=?", id); e != nil {
		return e
	}
...
func (s *Store) settleAgentTask(a Agent) error {
	...
	b, _ := json.Marshal(r)
	_, e = s.mutate(w.ID, "task.update", r.EventID, r.Revision, b, func(w *Work) error { return s.apply(w, "task.update", r) })
	if e != nil {

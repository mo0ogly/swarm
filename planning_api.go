...
func (s *Store) planningCLI(pos []string, input string, out io.Writer) error {
	...
	if len(pos) != 3 {
		return fmt.Errorf("usage : planning show|enable|claim|decide|handoff|step|configure-reviewer|retry-review|review-step|pause|resume WORK [--input requête.json]")
	}

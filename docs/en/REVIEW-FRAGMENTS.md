...
Tests use a deterministic local provider: passing verdict, final refusal, unpaid preflight retry without another producer, and replay without duplicate calls, a second publication on the accepted baseline, and invalidation when that baseline’s final journal is corrupted. **Explicit recovery of interrupted inspections preserves the same journal, successful inspections and every spent call.** It checks the remaining budget and consumes a durable authorization under the review lock. Reconciliation alone does not retry an interrupted call. Recovery of final calls remains held and must be addressed before deployment. The read-only preflight retains `executable:false`: it is not an execution permit.

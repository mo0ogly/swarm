...
Exploration initiale : un rapport ou un livrable pas encore créé est une absence attendue, pas une panne de configuration. Tester son existence avant lecture : if test -f docs/%[8]s.md; then cat docs/%[8]s.md; else printf 'Rapport non encore créé\n'; fi. Ne jamais lancer cat sur une liste de livrables facultatifs sans ce contrôle.
Lister les fichiers avec rg --files -g '*review*.go' -g '*recover*.go' ; garder les motifs entre guillemets, sans glob nu tel que ls *reprise*.go (le shell peut refuser un motif sans correspondance). Pour une recherche rg, distinguer son code 1 « aucun résultat » de son code 2 « erreur » : if rg --files -g '*review*.go'; then :; else code=$?; if test "$code" -eq 1; then printf 'Aucun fichier correspondant\n'; else exit "$code"; fi; fi. Adapter les motifs au besoin réel. Ne pas utiliser || true pour cacher un échec ; les tests et contrôles de validation doivent conserver leur code de sortie réel. Une recherche vide ne démontre ni un critère ni la réussite d’un contrôle.
...
Vérifier le répertoire courant avant une commande ; préférer les chemins absolus. Fixer un délai explicite pour chaque outil et commande, au plus %[2]d secondes ; découper les opérations longues.

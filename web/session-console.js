...
export class SessionConsole {
 ...
 append(text){
  ...
  text=text.replace(/^(\d{4}-\d{2}-\d{2}T[^ ]+) · (\w+) · /gm,(_,at,kind)=>{const time=new Date(at);return '['+(Number.isNaN(time.getTime())?at:time.toLocaleTimeString('fr-FR'))+'] '+({activity:'ACTIVITÉ',command:'DÉMARRAGE',lifecycle:'ÉTAT',limits:'LIMITES',error:'ERREUR',stderr:'ERREUR'}[kind]||kind.toUpperCase())+' · '});
 ...
 paint(){
  ...
  rows.forEach((line,index)=>{
   let role='';if(/\] (ERREUR|FAILED) · /i.test(line))role='error';else if(/\] (ACTIVITÉ|DÉMARRAGE) · /i.test(line))role='action';else if(/\] (LIMITES|WARNING) · /i.test(line))role='warning';else if(/\] ÉTAT · /i.test(line))role='lifecycle';
   if(role)decorations.push({range:new m.Range(index+1,1,index+1,line.length+1),options:{isWholeLine:true,className:'console-line-'+role,inlineClassName:'console-ink-'+role}});

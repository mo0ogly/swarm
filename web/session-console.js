const tr_web_session_console_js = source => globalThis.SwarmI18n?.t(source) ?? source;
...
export function readableSessionText(text){
 return text.split('\n').map(line=>{
  ...
  let event;try{event=JSON.parse(match[2])}catch{return match[1]+tr_web_session_console_js(' · warning · Détail capturé incomplet ; consulter la sortie originale.')}
  const results=[];
  if(event.type==='item.completed'&&event.item?.type==='command_execution'){
   results.push((event.item.exit_code===0?tr_web_session_console_js('Commande réussie'):tr_web_session_console_js('Commande terminée (code ')+event.item.exit_code+')')+'\n'+(event.item.aggregated_output||tr_web_session_console_js('Aucune sortie textuelle.')));
  }
  if(event.type==='user')for(const block of event.message?.content||[]){
   ...
   results.push((block.is_error?tr_web_session_console_js('Échec de l’outil'):tr_web_session_console_js('Réponse de l’outil'))+'\n'+(value||tr_web_session_console_js('Aucune sortie textuelle.')));
  }
...
export class SessionConsole {
 ...
 append(text){
  ...
  text=readableSessionText(text).replace(/^(\d{4}-\d{2}-\d{2}T[^ ]+) · (\w+) · /gm,(_,at,kind)=>{const time=new Date(at);return '['+(Number.isNaN(time.getTime())?at:time.toLocaleTimeString((globalThis.SwarmI18n?.locale || 'fr-FR')))+'] '+({message:'MESSAGE',result:tr_web_session_console_js('RÉSULTAT'),warning:'ATTENTION',activity:tr_web_session_console_js('ACTIVITÉ'),command:tr_web_session_console_js('DÉMARRAGE'),lifecycle:tr_web_session_console_js('ÉTAT'),limits:'LIMITES',error:'ERREUR',stderr:'ERREUR'}[kind]||kind.toUpperCase())+' · '});
 ...
 paint(){
  ...
  let latestAction=-1;
  rows.forEach((line,index)=>{if(/\] (MESSAGE|ACTIVITÉ|DÉMARRAGE) · /.test(line)&&!line.includes(tr_web_session_console_js("Résultat d'outil reçu")))latestAction=index});
  rows.forEach((line,index)=>{
   const match=line.match(/^(\[[^\]]+\]) ([^·]+) · (.*)$/);if(!match)return;
   const type=match[2].trim(),body=match[3];
   const role=/ERREUR|FAILED/.test(type)?'error':/LIMITES|ATTENTION|WARNING|MONITORING|OUTPUT-LIMIT/.test(type)?'warning':/ACTIVITÉ|DÉMARRAGE/.test(type)&&!body.includes(tr_web_session_console_js("Résultat d'outil reçu"))?'action':type==='MESSAGE'?'message':type===tr_web_session_console_js('RÉSULTAT')?'result':'lifecycle';
...
export function sessionProgress(r){
 ...
 if(r.status==='completed')return tr_web_session_console_js('L’exécution est terminée : le rapport reste à examiner');
 if(['failed','interrupted'].includes(r.status))return tr_web_session_console_js('L’exécution s’est arrêtée : examinez son motif et le rapport éventuel');
 if(r.desired==='stop'||r.status==='stopping')return tr_web_session_console_js('Arrêt demandé : confirmation attendue');
 if(h.process_state?.startsWith('unknown/')||h.process_state?.endsWith('/unconfirmed'))return tr_web_session_console_js('Activité de l’agent non confirmée');
 if(h.activity_state==='old')return tr_web_session_console_js('Aucune activité récente : progression à vérifier');
 if(p.pending_tools>0)return tr_web_session_console_js('En attente de la réponse d’un outil');
 return r.status==='running'?tr_web_session_console_js('En attente de la prochaine activité ou de la fin de l’agent'):tr_web_session_console_js('Démarrage en attente de confirmation');
}

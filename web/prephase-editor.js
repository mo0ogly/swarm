...
export class PreparationEditor {
 ...
 async initialize() {
  if(this.simple){this.modeMessage('Éditeur texte — adapté aux petits écrans.');return}
  try {
   const css=document.createElement('link');css.rel='stylesheet';css.href='/lib/monaco/editor.css';const cssReady=new Promise((resolve,reject)=>{css.onload=resolve;css.onerror=()=>reject(new Error('CSS indisponible'));});document.head.append(css);
   // Only the documented Monaco integration object is global.
   globalThis.MonacoEnvironment={getWorker:(_id,label)=>{const worker=new Worker(label==='json'?'/lib/monaco/json-worker.js':'/lib/monaco/worker.js',{type:'module'});worker.addEventListener('error',()=>{const text=this.value();this.simple=true;this.setText(text);this.showMode();this.modeMessage('Éditeur enrichi indisponible — votre texte reste éditable ici.');});return worker}};
   let timeout;try{const loaded=await Promise.race([Promise.all([import('/lib/monaco/editor.js'),cssReady]),new Promise((_,reject)=>{timeout=setTimeout(()=>reject(new Error('Chargement trop long')),15000)})]);this.monaco=loaded[0];}finally{clearTimeout(timeout);}
   this.theme();
   this.editor=this.monaco.editor.create(this.host,{model:null,theme:'wattson-preparation',automaticLayout:true,minimap:{enabled:false},fontSize:15,wordWrap:'on',maxTokenizationLineLength:2000,autoIndent:'none',autoClosingBrackets:'never',autoClosingQuotes:'never',autoSurround:'never',formatOnPaste:false,formatOnType:false,scrollBeyondLastLine:false,accessibilitySupport:'on',editContext:false,ariaLabel:'Document de préparation',tabIndex:0,renderLineHighlight:'none',stickyScroll:{enabled:false}});
   this.editor.onDidChangeModelContent(()=>{if(!this.updating){this.wrap(this.editor.getValue());this.onChange(this.editor.getValue())}});
   this.simple=false;this.showMode();this.modeMessage('Éditeur enrichi · Ctrl/Cmd + S pour enregistrer · Ctrl + M pour le mode Tab.');
   if(this.key)this.open(this.key,this.kind,this.plain.value);
  }catch(e){this.simple=true;this.showMode();this.modeMessage('Éditeur enrichi indisponible — votre texte reste éditable ici.');}
 }
 wrap(text){const long=text.split("\n").some(line=>line.length>2000);if(this.longLines!==long){this.longLines=long;this.editor?.updateOptions({wordWrap:long?"off":"on",wordWrapOverride1:long?"off":"inherit",stopRenderingLineAfter:-1});}this.modeMessage(long?"Éditeur enrichi · lignes longues : défilement horizontal, texte intégral conservé.":"Éditeur enrichi · Ctrl/Cmd + S pour enregistrer · Ctrl + M pour le mode Tab.");}
 ...
 showMode(){this.host.hidden=this.simple||!this.editor;this.plain.hidden=!this.host.hidden;document.getElementById('editor-toggle').textContent=this.simple?'Utiliser l’éditeur enrichi':'Utiliser l’éditeur texte';}
 async toggle(){const value=this.value();if(!this.editor){this.simple=false;await this.initialize();}else{this.simple=!this.simple;}this.setText(value);this.showMode();this.modeMessage(this.simple?'Éditeur texte · Ctrl/Cmd + S pour enregistrer.':'Éditeur enrichi · Ctrl/Cmd + S pour enregistrer · Ctrl + M pour le mode Tab.');}

'use strict';
const tr_web_conduite_js = source => globalThis.SwarmI18n?.t(source) ?? source;
...
function openDecision(d) {
  openModal(tr_web_conduite_js('Décision — ') + (d.task_id || 'Travail'),
    tr_web_conduite_js('L’acquittement ne valide pas la tâche et n’arrête pas l’agent.'), { action: 'decision', decision: d.id });
  preview(decisionObservation(d).summary + '\n' + d.evidence);
  field('note', tr_web_conduite_js('Décision motivée'), '', null, true, tr_web_conduite_js('Journalisée avec votre compte local et la date.'));
}

function decisionObservation(d) {
  const restored=d.kind==='silence'&&snapshot.pilotage?.health[d.agent_id]?.process_state==='running';
  return {label:restored?tr_web_conduite_js('Signal rétabli — incident à examiner'):(escalationLabels[d.kind]||d.kind),
    summary:restored?tr_web_conduite_js('Un signal récent est de nouveau reçu. L’incident enregistré reste à examiner ; aucun acquittement automatique.'):d.summary};
}
...
function agentLine(item) {
  ...
  parts.push((a.progress?.tool_calls || 0) + tr_web_conduite_js(' appels'));
  ...
  if (a.progress?.degraded) parts.push(tr_web_conduite_js('flux dégradé : ') + a.progress.degraded);
  return parts.join(' · ');
}
...
function planRow(t) {
  ...
  else if (t.status === 'todo' && (t.depends || []).length) row.append(node('p', tr_web_conduite_js('En attente de : ') + t.depends.join(', '), 'plan-wait'));
...
function renderConduite() {
  if (!snapshot) return;
  const levels = [['manuel', tr_web_conduite_js('Manuel — départs et relais humains')],
    ['assiste', tr_web_conduite_js('Assisté — relais automatique du handoff prouvé')],
    ['autonome', tr_web_conduite_js('Autonome — départs automatiques dans les créneaux')]];
  ...
  $('conduite-pause').textContent = snapshot.paused ? tr_web_conduite_js('Autoriser les départs') : tr_web_conduite_js('Suspendre les départs');
  ...
  const lines = [snapshot.autonomy_label || snapshot.autonomy,
    busy + tr_web_conduite_js(' créneau(x) occupé(s) sur ') + (snapshot.slots || 2),
    (snapshot.cost_text || tr_web_conduite_js('coût réel non rapporté')),
    tr_web_conduite_js('réservé : ') + budget.reserved_usd + ' + ' + budget.committed_estimate_usd + tr_web_conduite_js(' USD estimés sur ') + budget.budget.limit_usd];
  if (snapshot.paused) lines.push(tr_web_conduite_js('départs suspendus'));
  ...
  if (!open.length) inbox.append(node('p', tr_web_conduite_js('Aucune demande de décision ouverte. Les tâches bloquées et résultats à examiner restent accessibles par « Prochaine intervention ».')));
  ...
  if (!snapshot.work.tasks.length) plan.append(node('p', tr_web_conduite_js('Aucune tâche dans ce travail. Ouvrez le mode expert pour en ajouter.')));
}
...
function renderAccueil(ouvertes, budget) {
  ...
  if (change && change.entrees > 0) {
    const quoi = change.taches > 0
      ? change.taches + (change.taches > 1 ? tr_web_conduite_js(' tâches ont avancé') : tr_web_conduite_js(' tâche a avancé'))
      : change.entrees + (change.entrees > 1 ? tr_web_conduite_js(' événements') : tr_web_conduite_js(' événement'));
    // « au moins » tant que le trait de visite n'est pas dans la page chargée :
    // le total réel peut être plus grand.
    parts.push(segment((change.complet ? '' : tr_web_conduite_js('au moins ')) + quoi + tr_web_conduite_js(' depuis votre visite'), 'fil-titre'));
  }

  if (ouvertes.length) {
    parts.push(segment(ouvertes.length + (ouvertes.length > 1 ? tr_web_conduite_js(' décisions vous attendent') : tr_web_conduite_js(' décision vous attend')), 'conduite-inbox-title'));
  }

  if (!parts.length) {
    // Ni zéro ni tableau vide : une phrase qui dit franchement qu'il n'y a rien.
    hote.append(node('span', change ? tr_web_conduite_js('Rien de neuf depuis votre visite · aucune décision en attente')
                                    : tr_web_conduite_js('Aucune décision en attente')));
  } else {
    parts.forEach((p, i) => {
  ...
  const morceaux = [];
  if (tentatives > 0) morceaux.push(snapshot.cost_text || tr_web_conduite_js('coût réel non rapporté'));
  if (budget && budget.budget.limit_usd > 0) {
    morceaux.push(budget.reserved_usd + tr_web_conduite_js(' USD réservés sur ') + budget.budget.limit_usd);
  }
  if (morceaux.length) {
...
$('conduite-apply').onclick = async () => {
  try {
    ...
    await refresh(true);
    notice(tr_web_conduite_js('Réglage appliqué ; sans effet sur les tentatives déjà lancées.'));
...
$('conduite-dispatch').onclick = async () => {
  try {
    ...
    await refresh(true);
    notice(r.message || tr_web_conduite_js('Ordonnancement demandé.'));
  } catch (e) { notice(e.message, true) }
};
{

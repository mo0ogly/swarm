...
const Mission={
 ...
 primary(o){
  ...
  if(o.kind==='decision'){if(snapshot.task_actions?.[o.task.id]?.some(a=>a.kind==='extend-attempt'&&a.disponible)){PilotActions.extensionDialog(o.task.id);return}if(o.task.state==='review')this.action(o.task);else MissionHelp.open(o.task.id);return}
 ...
 render(){
  ...
  const actionable=d.tasks.filter(t=>['review','intervention'].includes(t.state)||snapshot.task_actions?.[t.id]?.some(a=>a.kind==='extend-attempt'&&a.disponible)).sort((a,b)=>b.impact-a.impact);
  for(const t of actionable.slice(0,3)){const row=node('section',undefined,'mission-intervention');row.append(node('h4',t.title));const extension=PilotActions.extensionButton(t.id);if(extension)row.append(extension);row.append(this.resultView(t.result),this.understandingView(t.understanding));const diagnostic=this.diagnosticView(t.diagnostic,'priority-'+t.id);if(diagnostic)row.append(diagnostic);if(t.impact)row.append(node('p',t.impact+tr_web_mission_js(' tâche(s) dépendent de ce résultat.')));const b=Pilot.command(missionText(t.label),()=>this.action(t));b.dataset.missionAction=t.id;row.append(b,this.helpButton(t));host.append(row)}

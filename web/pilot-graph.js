'use strict';
const tr_web_pilot_graph_js = source => globalThis.SwarmI18n?.t(source) ?? source;
...
const PilotGraph = (() => {
 ...
 function taskSession(agents,taskID) {
  ...
  return {agent:item.agent,label:running?tr_web_pilot_graph_js('Voir l’agent travailler'):tr_web_pilot_graph_js('Voir la session')};
 }
 ...
 function guidance(task,go,validation) {
  if(validation?.state==='accepted')return tr_web_pilot_graph_js('✓ Résultat validé');
  if(task.status==='abandoned')return tr_web_pilot_graph_js('Tâche abandonnée');
  if(task.status==='submitted')return tr_web_pilot_graph_js('À examiner : le résultat reçu');
  if(task.status==='blocked')return tr_web_pilot_graph_js('À résoudre : ')+(task.blocker||go?.reason||tr_web_pilot_graph_js('examiner la tentative'));
 ...
 function organization(work,tasks,paused=false){
  ...
  const nodes=(p.scopes||[]).map(s=>({id:'@scope/'+s.id,kind:'planner',role:s.parent?'subplanner':'planner',scope:s.id,tone:s.parent?'attention':'info',title:s.parent?tr_web_pilot_graph_js('⑂ Sous-planificateur'):tr_web_pilot_graph_js('◇ Orchestrateur · ')+(s.model_selection?.provider||p.provider),scopeLabel:s.parent?(s.objective||s.id).split('\n')[0].replace(/^Responsable — /,''):'',description:paused||p.paused?tr_web_pilot_graph_js('En pause'):s.holder?tr_web_pilot_graph_js('Décision en cours'):s.state==='closed'?tr_web_pilot_graph_js('Périmètre terminé'):tr_web_pilot_graph_js('Attend les retours'),detail:(s.model_selection?.route?.model||p.model_route?.model||'auto')+' · '+(s.requirements||[]).length+tr_web_pilot_graph_js(' exigences · décide sans coder')}));
  nodes.push({id:'@reviewer',kind:'reviewer',tone:p.reviewer&&!p.reviewer.failure?'info':'attention',title:p.reviewer?tr_web_pilot_graph_js('✓ Vérificateur IA · ')+p.reviewer.provider:p.repository?tr_web_pilot_graph_js('✓ Contrôleur du moteur'):tr_web_pilot_graph_js('! Vérificateur IA absent'),description:p.reviewer?(paused?tr_web_pilot_graph_js('En pause'):p.reviewer.failure?tr_web_pilot_graph_js('Vérification interrompue'):tasks.some(t=>t.independent_review?.state==='running')?tr_web_pilot_graph_js('Examen en cours'):tr_web_pilot_graph_js('Attend les résultats')):p.repository?tr_web_pilot_graph_js('Tests et intégration Git'):tr_web_pilot_graph_js('À configurer'),detail:p.reviewer?(p.reviewer.model_route?.model||'auto')+' · '+p.reviewer.calls+'/'+p.reviewer.max_calls+tr_web_pilot_graph_js(' appels · session indépendante'):p.repository?tr_web_pilot_graph_js('Aucune revue IA indépendante'):tr_web_pilot_graph_js('Aucun avis IA ne sera inventé')});

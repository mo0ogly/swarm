...
(async()=>{
 ...
 await select('#pilot-detail','detailed');assert.equal(await page.$eval('.graph-noeud',e=>e.querySelectorAll('[data-line]').length),8);
 ...
 for(let i=0;i<8;i++)await page.click('#pilot-zoom-in');

...
cli('init');
const provider=path.join(root,'terminal-fixture.py');fs.writeFileSync(provider,`import sys,os,signal,fcntl,termios,struct
assert sys.stdin.isatty()
print("\\033[32mSESSION INTERACTIVE — RECETTE\\033[0m",flush=True)
print("Aucun modèle appelé. Écrivez un message ; /exit termine la session.",flush=True)
for line in sys.stdin:
 if line.strip()=="/exit":break
 print("\\033[36mAGENT > "+line.strip()+"\\033[0m",flush=True)
`);
fs.writeFileSync(path.join(root,'.swarm/providers.json'),JSON.stringify({schema_version:1,providers:{recette:{command:'/usr/bin/python3',args:[provider],interactive_args:[provider],env_allow:[]}}}));
...
(async()=>{
 ...
 await frame.waitForFunction(()=>document.querySelector('#terminal-screen').textContent.includes('AGENT > Bonjour depuis la modale'));
 checks.push('lancement via formulaire, vrai PTY, couleurs ANSI, saisie aller-retour');
 ...
 await page.click('.graph-noeud');await page.waitForSelector('#agent-terminal-dialog[open]');frame=await(await page.$('#agent-terminal-dialog iframe')).contentFrame();await frame.waitForFunction(()=>document.querySelector('#terminal-screen').textContent.includes('AGENT > Bonjour'));

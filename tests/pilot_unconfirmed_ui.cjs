...
(async()=>{
 ...
 assert.match(await page.$eval('#pilot-inspector .pilot-validation',e=>e.textContent),/Démarrage non confirmé — arrêt demandé/);
 assert.doesNotMatch(await page.$eval('#pilot-inspector-body',e=>e.textContent),/En cours/);
 assert.match(await page.$eval('.graph-noeud[data-task=review-codex] [data-line="1"]',e=>e.getAttribute('aria-label')),/Démarrage non confirmé/);
 assert.equal(await page.$eval('.graph-noeud[data-task=review-codex]',e=>e.dataset.etat),'attention');

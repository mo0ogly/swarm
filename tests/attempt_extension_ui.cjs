...
(async()=>{
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(failed,[]);const result={status:'PASS',root,checks:['visible priority and inspector action','French/English × dark/light','required fields','Escape restores focus','cancel preserves state','stale revision refused','authorization persisted','history and review retained','100-tool ceiling retained','no provider started','extension hidden while the authorized third attempt remains','mobile layout','no unexpected browser errors']};fs.writeFileSync(path.join(out,'result.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result));

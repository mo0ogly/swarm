...
(async()=>{
 ...
 for(const lang of ['fr','en']){
  ...
  for(const theme of ['sombre','etat']){
   ...
   await page.waitForSelector('[data-recovery-action="resume-launch"]');await page.click('[data-recovery-action="resume-launch"]');
 ...
 await page.click('[data-recovery-action="resume-launch"]');await page.waitForSelector('#prepared-launch-summary');
 ...
 const result={status:'PASS',checks:['CLI discovers original preparation','incomplete delivery diagnosis and attributed report visible','visible inspector recovery button','French/English and dark/light','help modal','Escape restores focus','cancel retains state','mobile layout','web resumes original launch','CLI replay does not duplicate provider','one attempt, no acceptance','no browser errors']};fs.writeFileSync(path.join(out,'result.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result));

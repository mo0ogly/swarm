...
(async()=>{
 ...
 await page.evaluate(()=>{Pilot.state.view='dependencies';Pilot.state.search='';Pilot.state.filter='all';Pilot.changed();Pilot.inspect('task','review-claude-host')});
 ...
 assert.match(await page.$eval('.pilot-mission',e=>e.textContent),/superviseur hôte/);
 assert.match(await page.$eval('#pilot-inspector-body',e=>e.textContent),/Résultat public reçu — tentative terminée/);
 assert.doesNotMatch(await page.$eval('#pilot-inspector-body',e=>e.textContent),/progression non démontrée/);
 for(const theme of ['etat','sombre']){
  ...
  await page.focus('#pilot-inspector button');await page.screenshot({path:path.join(out,'completed-'+theme+'.png')});
 }
 ...
 console.log('PASS: revue réelle sans dépendances, activité après fin confirmée, thèmes résolus, focus, refresh et Échap.');

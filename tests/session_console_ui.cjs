...
(async()=>{
 ...
 browser=await puppeteer.launch({headless:true,executablePath:'/usr/bin/google-chrome',args:['--no-sandbox']});const page=await browser.newPage(),csp=[];page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(/Content Security Policy|Refused to/.test(m.text()))csp.push(m.text())});page.setDefaultTimeout(20000);await page.setViewport({width:1440,height:1100});await page.goto(url);await page.waitForSelector('#pilot-mission');await page.select('#work',dense.id);await page.waitForFunction(()=>snapshot?.agents?.some(x=>x.agent.task_id==='g2'));
 await page.evaluate(()=>AgentTerminal.open(snapshot.agents.find(x=>x.agent.task_id==='g2').agent));await page.waitForSelector('#agent-terminal-dialog iframe');const frame=await(await page.$('#agent-terminal-dialog iframe')).contentFrame();await frame.waitForSelector('#session-console .monaco-editor');await frame.waitForFunction(()=>document.querySelector('#session-console-fallback').textContent.includes('ACTIVITÉ'));
 assert.match(await frame.$eval('#console-mode-label',e=>e.textContent),/Journal de l’agent/);assert.equal(await frame.$eval('#terminal-screen',e=>e.hidden),true);assert.equal(await frame.$eval('#terminal-claim',e=>e.disabled),true);assert.match(await frame.$eval('#terminal-status',e=>e.textContent),/Agent automatisé/);assert.match(await frame.$eval('#session-cost',e=>e.textContent),/Coût inconnu/);assert.ok(await frame.$('.console-line-action'));
 for(const theme of ['etat','sombre']){await page.evaluate(t=>document.documentElement.dataset.theme=t,theme);await frame.waitForFunction(t=>document.documentElement.dataset.theme===t,{},theme);const missing=await frame.evaluate(()=>{const s=getComputedStyle(document.documentElement);return ['info-fond','info-encre','attention-fond','attention-encre','alerte-fond','alerte-encre','champ','texte'].filter(x=>!s.getPropertyValue('--wattson-'+x).trim())});assert.deepEqual(missing,[]);await page.screenshot({path:path.join(outDir,theme+'.png'),fullPage:true})}
 ...
 const ended=await frame.evaluate(async()=>{const {sessionProgress}=await import('/session-console.js');return ['failed','interrupted','completed'].map(status=>sessionProgress({status,progress:{pending_tools:1},desired:'stop'}))});for(const text of ended)assert.doesNotMatch(text,/attente/);
 const fallback=await browser.newPage();await fallback.setRequestInterception(true);fallback.on('request',r=>r.url().includes('/lib/monaco/editor.js')?r.abort():r.continue());await fallback.goto(frame.url());await fallback.waitForFunction(()=>document.getElementById('console-mode-label').textContent.includes('affichage simplifié'));assert.match(await fallback.$eval('#session-console-fallback',e=>e.textContent),/ACTIVITÉ/);await fallback.close();
 assert.deepEqual(errors,[]);assert.deepEqual(csp,[]);assert.equal(cli('agent','list',dense.id).agents.length,1);const result={status:'PASS',checks:['vrai agent automatisé, aucun modèle appelé','console Monaco par défaut','sorties colorées, numéros de ligne et recherche','saisie désactivée en mode automatisé','suivi automatique débrayable','terminal brut toujours accessible','deux thèmes, jetons et CSP vérifiés','Échap ferme la recherche avant la session','Monaco indisponible : repli texte sans perte de lecture','fin prioritaire sur outil sans réponse'],errors,csp};fs.writeFileSync(path.join(outDir,'ui.json'),JSON.stringify(result,null,2));console.log(result);

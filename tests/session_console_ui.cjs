...
(async()=>{
 ...
 browser=await puppeteer.launch({headless:true,executablePath:process.env.CHROME_BIN||'/usr/bin/google-chrome'});const page=await browser.newPage(),csp=[];page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(/Content Security Policy|Refused to/.test(m.text()))csp.push(m.text())});page.setDefaultTimeout(20000);await page.setViewport({width:1440,height:1100});await page.goto(url);await page.waitForSelector('#pilot-mission');await page.select('#work',dense.id);await page.waitForFunction(()=>snapshot?.agents?.some(x=>x.agent.task_id==='g2'));

...
(async()=>{
 ...
 await page.click('[data-view="agents"]');await has('#agents-list','subplanner');await page.click('#agents-list button');await page.select('#field-action','retry');await confirm();await has('#active-count','1');await page.click('[data-view="tasks"]');await openTask('UI-01','stop');const options=await page.$$eval('#field-agent option',es=>es.map(e=>({value:e.value,text:e.textContent})));const running=options.find(x=>/En cours|Démarrage|En attente|queued\/unconfirmed/.test(x.text));assert.ok(running,JSON.stringify(options));await page.select('#field-agent',running.value);await page.click('#cancel');await has('#active-count','1');await openTask('UI-01','stop');await page.select('#field-agent',running.value);await confirm();await has('#active-count','0');checks.push('Retry preserves role; cancel stop has no effect; selected attempt stopped');
 ...
 await openTask('UI-02');await page.select('#field-provider','recette');await page.click('#confirm');await has('#modal-error','dépendance');await page.click('#cancel');
 await page.click('[data-view="decisions"]');await page.locator('button::-p-text(Revalider cette tâche)').click();await has('#preview','Preuve modifiée');assert.equal(await page.$eval('#field-action',e=>e.value),'todo');await confirm();
 ...
 await openTask('UI-01','gate');await page.click('#confirm');await has('#modal-error','nouveau rapport');await page.click('#cancel');
 ...
 await openTask('UI-01','gate');await page.click('#confirm');await has('#confirm','Confirmer l’enregistrement');await confirm();
